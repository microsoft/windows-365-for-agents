...
internal sealed class TurnEnrichmentProcessor : BaseProcessor<LogRecord>
{
    public override void OnEnd(LogRecord data)
    {
        ...
        {
            var tags = turn.Tags;
            var enriched = new List<KeyValuePair<string, object?>>(data.Attributes ?? Array.Empty<KeyValuePair<string, object?>>())
            {
                new("tenant_id", tags.TenantId),
                new("agent_user", tags.AgentUser ?? "(none)"),
                new("user_oid", tags.UserOid ?? "(none)"),
                new("conversation_id", tags.ConversationId),
                new("session_id", tags.SessionId ?? "(none)"),
            };
